    ...
    def create_payslip(self):
        ...
        for att_sheet in self:
            ...
            if att_sheet.sheet_action == 'deduct_leave':
                for workd in worked_days_line_ids:
                    if workd and workd['sequence'] == 1:
                        total_hours = att_sheet.late_policy_hours + att_sheet.tot_absence + att_sheet.diff_policy_hours + att_sheet.forget_hours
                        days = math.floor(total_hours / 24)
                        workd['number_of_days'] -= days
                        workd['number_of_hours'] -= math.floor(total_hours)
                        break
            else:
                if att_sheet.sheet_action == 'deduct_payslip':
                    # overtime
