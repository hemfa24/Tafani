...
class GeneratingClosingEntry(models.TransientModel):
    _name = "generating.closing.entry"
    ...
    def data_save(self):
        ...
        for data in self:
            ...
            print("result 9777  7 77 7 7", result)
            ...
            acco_query = """
                ...
                WHERE a.account_type not in ('view', 'consolidation')
                  ...
                  AND a.close_method = '{close_method}' ORDER BY id asc""".format(
                      company_id=company_id,
                      close_method="unreconciled"
                )
            print("acco_query", acco_query)
            self._cr.execute(
                acco_query
            )
            result = self._cr.dictfetchall()
            print ("resultresult 1500000000", result)
            ...
            if account_ids:
                ...
                self._cr.execute(
                    ...
                    INSERT INTO account_move_line (
                         name, create_uid, create_date, write_uid, write_date, display_type,
                         statement_id, journal_id, currency_id, date_maturity,
                         partner_id, blocked, balance, debit, credit, parent_state,
                         ref, account_id, period_id, date, move_id, amount_currency,
                         quantity, product_id, company_id, fiscalyear_id)
                      (SELECT name, create_uid, create_date, write_uid, write_date, display_type,
                         statement_id, %s, currency_id, date_maturity, partner_id,
                         blocked, balance, debit, credit, 'draft', ref, account_id,
                ...
                self._cr.execute(
                    ...
                    INSERT INTO account_move_line (
                         ...
                         quantity, product_id, company_id, fiscalyear_id)
                      (SELECT
                         b.name, b.create_uid, b.create_date, b.write_uid, b.write_date, b.display_type,
                         b.statement_id, %s, b.currency_id, b.date_maturity,
                         b.partner_id, b.blocked, b.balance, b.debit, b.credit, 'draft',
            ...
            for account in obj_acc_account.with_context(
                ...
                print ("account_debit_line_ids ", account_debit_line_ids)
                print ("account_credit_line_ids ", account_credit_line_ids)
                ...
                if not company_currency_id.is_zero(abs(amount_balance)):
                    ...
                    if amount_balance < 0.0:
                        ...
                        print("amount_currency_total", amount_currency_total)
                        query_2nd_part_args += (
